#!/usr/bin/env python3
"""
Train Q-TCRNet only - Quick version for research paper
~2 hours training time

Usage:
    python -u run_qtcrnet_only.py
"""
...
def main():
    print("="*70)
    print(" Q-TCRNet Training - Research Paper Version")
    print("="*70)
    ...
    config = load_config()
    output_dir = Path('./experiments/qtcrnet_final')
    ...
    for epoch in range(num_epochs):
        ...
        for voxels, labels in train_loader:
            voxels = voxels.to(device) + torch.randn_like(voxels.to(device)) * train_noise_std
        ...
        with torch.no_grad():
            for voxels, labels in val_loader:
                voxels = voxels.to(device) + torch.randn_like(voxels.to(device)) * test_noise_std
                wave_labels = labels['waveform'].to(device)
                volt_labels = labels['voltage'].to(device)
                wave_out, volt_out = model(voxels)
                loss = criterion(wave_out, wave_labels) + criterion(volt_out, volt_labels)
